...
const currentpc = function(ci) {
    if (process.env.LUA_USE_APICHECK && !(ci.callstatus & lstate.CIST_LUA)) throw Error("assertion failed");
...
const upvalname = function(p, uv) {
    if (process.env.LUA_USE_APICHECK && !(uv < p.upvalues.length)) throw Error("assertion failed");
...
const lua_getinfo = function(L, what, ar) {
    ...
    if (what[0] === '>'.charCodeAt(0)) {
        ...
        if (process.env.LUA_USE_APICHECK && !(L, func.ttisfunction())) throw Error("function expected");
        ...
        func = ci.func;
        funcOff = ci.funcOff;
        if (process.env.LUA_USE_APICHECK && !(ci.func.ttisfunction())) throw Error("assertion failed");
    }
